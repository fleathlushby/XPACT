...
initial_perf_densenet_arduino = model.fit(train_data, train_labels, batch_size=16, validation_data=(test_data,test_labels), epochs=200, verbose=1)

pred_values = model.predict(test_data)
pred_values = np.round(pred_values).flatten()

with open("initial_perf_densenet_arduino.pkl", "wb") as file:
        pickle.dump(initial_perf_densenet_arduino.history, file)

model.save("initial_perf_densenet_arduino.h5")
...
with open("initial_perf_densenet_arduino.pkl", "rb") as file:
