...
initial_perf_xception_biochip_mixed = model.fit(train_data, train_labels, batch_size=32, validation_data=(test_data,test_labels), epochs=200, verbose=1)

pred_values = model.predict(test_data)
pred_values = np.round(pred_values).flatten()

with open("initial_perf_xception_biochip_mixed.pkl", "wb") as file:
        pickle.dump(initial_perf_xception_biochip_mixed.history, file)

model.save("initial_perf_xception_biochip_mixed.h5")
...
with open("initial_perf_xception_biochip_mixed.pkl", "rb") as file:
...
plt.title('Supervised training of Arduino UNO (original vs. counterfeit instances)', fontsize= 20, weight='bold')
